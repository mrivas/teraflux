...
def getAdditionalConstraint(model,v_dic,knownFluxes): 
    ...
    for i in knownFluxes.index:
        ...
        flux   = knownFluxes.iloc[i]["Reaction_Flux"]     
...
#################################################################################
### OPTIMIZATION
def optPheFlux(model,fpkmDic,k,init_time,knownFluxes):
    ##############################################################
    ...
    options={"ipopt":{"print_level":3,'max_iter': 15000}}
...
def getFluxes(inputFileName, processDir, prefix_log, verbosity):
    ...
    for i in range(nRows):
        ...
        k = 1000
        fluxes,optimization_time,total_time,status,success,lbx,ubx = optPheFlux(model,fpkmDic,k,init_time,knownFluxes)
